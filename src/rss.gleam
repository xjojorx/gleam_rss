...
fn parse_site(input: xmlm.Input) -> Result(Feed, String) {
  ...
  case xmlm.signal(input) {
    ...
    Ok(#(ElementStart(Tag(Name(_, tag_name), _)), new_input)) -> 
      case input_element(new_input, tag_name) {
        Error(err) -> Error(err)
        Ok(#(SimpleElement("title", content), input)) -> do_parse_article(input, Article(..article, title: content))
        Ok(#(SimpleElement("link", content), input)) -> do_parse_article(input, Article(..article, link: content))
        Ok(#(SimpleElement("pubDate", content), input)) -> do_parse_article(input, Article(..article, date: content))
        Ok(#(SimpleElement("description", content), input)) -> do_parse_article(input, Article(..article, description: content)) //TODO: handle cdata?
        Ok(#(SimpleElement("category", content), input)) -> do_parse_article(input, Article(..article, categories: [content, ..article.categories]))
...
fn input_element(input: Input, tag: String) -> Result(#(SimpleElement, Input), String) {
...
pub fn main() {
  ...
  let articles = read_articles_file("eurogamer.rss")
